...
let
  ...
  autogeneratedPassword =
    (config.enrollFido2 || config.enrollTpm2) && !(config.settings ? "keyFile");
  ...
  cryptsetupOpen = ''
    ...
    ${createOpenCommand {
      inherit keyFileArgs;
      tokenType = if config.enrollFido2 then "systemd-fido2" else if config.enrollTpm2 then "systemd-tpm2" else null;
    }}
  '';
...
in
{
  options = {
    ...
    enrollTpm2 = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Whether to enroll a TPM2 chip and use it";
    };
    extraTpm2EnrollArgs = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [ "--tpm2-pcrs=7" ];
      example = [
        "--tpm2-pcrs=0+7+15"
    ...
    _create = diskoLib.mkCreateOption {
      ...
      default = ''
        ...
        # Unlock without relying on the autogenerated password when someone
        # calls this script after unmounting or rebooting
        ...
        ++ (lib.optional config.initrdUnlock [
          {
            boot.initrd.luks.devices.${config.name} = {
              ...
              crypttabExtraOpts =
                lib.optionals config.enrollFido2 [ "fido2-device=auto" ]
                ++ lib.optionals config.enrollTpm2 [ "tpm2-device=auto" ];
            }
            // config.settings;
        ...
        ++ (lib.optional config.enrollTpm2 [
          {
            systemd.services."disko-tpm2-enroll-${config.name}" = {
              ...
              wants = ["tpm2.target"];
              unitConfig = {
                ConditionPathExists = "!/var/lib/luks-tpm-enroll/done-${config.name}";
              };
              ...
              script = ''
                ...
                echo "Mark tpm2 enrollment complete"
                mkdir -p /var/lib/luks-tpm-enroll
                touch /var/lib/luks-tpm-enroll/done-${config.name}
